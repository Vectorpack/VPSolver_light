...
Arcflow::Arcflow(const Instance &_inst, const bool verbose) {
	ready = false;
	tstart = CURTIME;
	init(_inst, verbose);
...
Arcflow::Arcflow(const char *fname, const bool verbose) {
	ready = false;
	tstart = CURTIME;
	init(fname, verbose);
	throw_assert(ready == true);
...
void Arcflow::init(const Instance &_inst, const bool verbose) {
...
void Arcflow::init(const char *fname, const bool verbose) {
	throw_assert(ready == false);
	if (check_ext(fname, ".vbp")) {
		init(Instance(fname), verbose);
...
void Arcflow::read(FILE *fin) {
	throw_assert(ready == false);
	tstart = CURTIME;
	//inst = Instance(fin); // This is wrong as 'fin' points to a .afg file
